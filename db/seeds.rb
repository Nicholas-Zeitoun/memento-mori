...
meme_config.each do |meme|
  # Assigns user
  meme_owner = User.where(username: meme.last["user"])[0]
  # Assigns category
  meme_category = Category.where(name: meme.last["category"])[0]
  new_meme = Meme.new(
    title: meme.last["title"],
    image_url: meme.last["image_url"],
    user: meme_owner,
